...
path = Path("../planetImageryCentroidForests").rglob("*.tif")
out_path = "../planetSinglesForests" 
for img_p in path:
    m1img_out_path = out_path +  str(img_p)[31:-4]+'m1'+str(img_p)[-4:]
    ...
    profile = src.profile
    profile.update(dtype=rs.uint8,count=4)
    
    with rs.open(m1img_out_path, 'w', **profile) as dst:
        dst.write(m1.astype(rs.uint8),4)
...
path = Path("../planetImageryCentroidOrchards").rglob("*.tif")
